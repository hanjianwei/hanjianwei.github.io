...
如果 `x` 是 `D` 的一个实例，那么 `x.show()` 到底会调用哪个 `show` 方法呢？如果按照 `[D, B, A, C]` 的搜索顺序，那么 `x.show()` 会调用 `A.show()`；如果按照 `[D, B, C, A]` 的搜索顺序，那么 `x.show()` 会调用 `A.show()`。由此可见，MRO 是把类的继承关系线性化的一个过程，而线性化方式决定了程序运行过程中具体会调用哪个方法。既然如此，那什么样的 MRO 才是最合理的？Python 中又是如何实现的呢？
...
[diamond]: class_diamond.svg "菱形继承"
[new_diamond]: newclass_diamond.svg "新式类菱形继承"
[class_conflict]: class_conflict.svg "类型冲突"
[c3_example]: c3_example.svg "C3例子"
